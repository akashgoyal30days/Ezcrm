...
//fetch state
  ...
  Future go_for_lead(String usrid, String client, String status, String l_start, String l_end) async{
    lead_data = await Get_Leads(usrid, client, status, l_start, l_end);
    if (lead_data.containsKey('status')) {
      if (lead_data['status'] == true) {
        setState(() {
          myleaddatalist = lead_data['data'];
        });
        if(debug == 'yes'){
                            ...
                            child: ButtonTheme(
                              child: DropdownButton(
                                elevation: 0,
                                  ...
                                  dropdownColor: Colors.white,
                                  hint: Text('Search Leads',style: TextStyle(fontSize: 18,color: Colors.blue),),
                                  value: _valuenew,
                                items: expnewdata?.map((item) {
                                  return new DropdownMenuItem(
                                    child: new Text(item['status_label'],style: TextStyle(fontSize: 18,color: Colors.blue),),
                                    value: item['status_label'].toString(),
                                  );
                                })?.toList() ??
                                    [],
                                onChanged: (newValue){
